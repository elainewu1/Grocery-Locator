...
public class CR extends JFrame {
	...
	public CR() {
		...
		JLabel lblCategoryI = new JLabel("Consumption Requirement ID");
		lblCategoryI.setBounds(15, 26, 147, 20);
		contentPane.add(lblCategoryI);
		
		JLabel lblCategoryName = new JLabel("Consumption Requirement Name");
		...
		JButton btnInsertCategory = new JButton("Insert Consumption Requirement");
		btnInsertCategory.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					String sql = "insert into wu_elaine_db.Consumption_Requirement(Consumption_ID,"
							+ "Consumption_Name) values("
							+ id +",'"+name+"')";
					DataAccess da = new DataAccess();
					
					int result = da.ExecuteUpdate(sql);
					if (result>0) {
						lblMess.setText("Consumption Requirement Added.");
					
					}
					else {
						lblMess.setText("Failed to add requirement.");
					}
		...
		btnUpdateCategory.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					...
					String sql = "update `Consumption_Requirement` set `Consumption_Name`=? where `Consumption_ID`=?";
					
					//checking if it exists in database
					String sql_s = "Select * from Consumption_Requirement where Consumption_ID="+id;
					DataAccess da = new DataAccess();
					ArrayList <String> temp = da.test(sql_s);
					if (temp.isEmpty()) {
						lblMess.setText("The requirement ID does not exist");
					}
					else {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String sql = "delete from `Consumption_Requirement` where `Consumption_ID`=?";
